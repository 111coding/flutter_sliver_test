...
class EasySliverAppBar extends StatelessWidget {
  ...
  EasySliverAppBar({required this.avatar, required this.username, required this.token, this.extent = 250, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return SliverPersistentHeader(
      pinned: true,
      delegate: EasySliverDelegate(avatar: avatar, username: username, token: token, extent: extent > 200 ? extent : 200),
    );
  }
}

class EasySliverDelegate extends SliverPersistentHeaderDelegate {
  ...
  EasySliverDelegate({required this.avatar, required this.username, required this.token, this.extent = 250}) : assert(extent >= 200);
  ...
  @override
  bool shouldRebuild(EasySliverDelegate oldDelegate) {
